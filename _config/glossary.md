...
| **all_jobs.json** | The 14-day rolling master of every scraped role, with `first_seen` timestamps. Exists because per-source files are short rolling windows (e.g. `linkedin_jobs.json` holds only ~the last hour). Every scraper run merges its `new_jobs` into this master. |
...
| **first_seen** | Timestamp added when a job first appears in `all_jobs.json`. Used by `--since N` and the 14-day rolling window. |
